...
::: {.callout-solution}
Der findes 3 isozymer af LDH i mennesker (A chain, B chain or C chain).
:::

I Berg betegnes to former af LDH som henholdsvis H og M form.

2. Hvordan betegnes disse to varianter i Uniprot?

::: {.callout-solution}
"A chain" er muskelformen (M), hvilket ses under "*Names and taxonomy, Protein names*" mens "B chain" er hjerteformen (H). "C chain" er en speciel form i testes (LDH-X).
:::

3. På hvilke kromosomer finder man generne for de to former?

::: {.callout-solution}
Genet for LDH-M ("A chain") findes på kromosom 11 (står under "*Names and taxonomy*"), mens genet for LDH-H ("B chain") findes på kromosom 12.
:::

4. Hvor mange splejsningsvarianter kendes for LDH-M?

::: {.callout-solution}
LDH-M ("A chain") udviser 5 splejsningsvarianter (isoform 1\...isoform 5). 
:::
...
::: {.callout-solution}
PALA er en bisubstrat analog og bør derfor inhibere [kompetitivt]{.underline}. 10 nM er en stærk binding, så PALA er en stærk inhibitor.
:::

Stimulering af ATCase-aktiviteten kan observeres under visse eksperimentelle betingelser, som vist nedenfor.

![](figures/te_12/opgave-2-figur-1.png){width="60%" fig-align="center" .lightbox}

2. Beskriv kurven og forklar hvad der foregår.

::: {.callout-solution}
ATCase er et multisubunit-enzym, der udviser kooperativ substratbinding. Derfor vil tilsætning af PALA i først omgang stimulere enzymets aktivitet (op til 3 per ATCase, med 6 aktive sites), da det binder til nogle aktive sites og dermed aktiverer de andre (T-R transition). Men ved højere koncentrationer bliver effekten af inhiberingen tydelig, da de aktive sites efterhånden bliver besat med PALA og ikke kan fungere.
:::
...
::: {.callout-solution}
Pathway ses nedenfor. Trinnet der er blokeret i mutanten er trin C (enzymet hedder homoserine dehydrogenase). Dette medfører at aspartate semialdehyde (ASA) ophobes fordi dannelsen af lysin ikke kan aftage alt det ASA der produceres. Den overskydende ASA, ligner Asp og inhiberer derfor ATCase. ATCase varetager det første ("committede") reaktionstrin på vej til uracil, der er precursor for både UTP og CTP. Når der tilsættes uracil til vækstmediet er ATCase reaktionen unødvendig og væksten normal.

![](figures/te_12/opgave-2-figur-2.jpeg){width="90%" fig-align="center" .lightbox}

Berg 10. udgave s. 214 (Fig. 7.7) viser det aktive site af ATCase. En lysin sidekæde (Lys-84) koordinerer carboxylsyregruppen i substratet (Asp). Hvis substratanalogen ASA er bundet vil aldehyden danne en Schiff's base med Lys-84 og dermed inaktivere enzymet irreversibelt gennem kovalent modifikation. 
:::
...
::: {.callout-solution}
PALA binder til de katalytiske trimerer mens ATP binder til de regulatoriske dimerer.
:::

3. Hvorfor har PALA været specielt nyttig i forhold til at forstå ATCases egenskaber?

::: {.callout-solution}
PALA er et bisubstrat samt transition state analog, har illustreret de konformationelle ændringer, der finder sted.
:::

4. Forklar hvordan PALA kan fungere både som en aktivator og en inbitor af ATCase.

::: {.callout-solution}
PALA promoverer T til R transitionen og fungerer dermed som en aktivator af enzymet. Men PALA er også en transition state-analog, der inhiberer enzymet.
:::
...
::: {.callout-solution}
Thrombin kløver mellem Arg og Gly og aktivering af prothrombin kræver spaltning af to peptidbindinger, mellem Arg-Thr og Arg-Ile.
:::

2. Beskriv ligheder og forskelle mellem den måde de to enzymer genkender deres substrat på.

::: {.callout-solution}
Mindre plads i aktivt site, brug for fleksibilitet fra Gly eller S1' site specifikt for Gly.
:::
...
![](figures/te_12/opgave-4-figur-1.png){width="50%" fig-align="center"}

3. Hvilken gruppe af stoffer tilhører heparin og hvilke funktionelle grupper indeholder det?

::: {.callout-solution}
Heparin er en negativt ladet polysaccharid eller glycosaminoglycan med funktionelle O-SO~3~^-^ grupper. Fungerer som en antikoagulant ved at stabilisere serpin-protease-interaktionen.
:::

4. Hvilken type af sidekæder vil du forvente at heparin binder til? 

::: {.callout-solution}
På grund af heparins mange negative ladninger binder det til basiske (positivt ladede) aminosyresidekæder i proteiner. Hexose ringen har en hydrophob flade som kan vekselvirke med hydrophobe sidekæder. Sekvensmotivet XBBXBX-motiv, hvor B er en basisk og X en hydrofob eller uladet aminosyre, ses derfor ofte at være heparin bindende.
:::

5. Lav en scene, kaldet F1, som henter thrombin (PDB-ID: `3UTU`). Lav så en scene, kaldet F2, som viser den elektrostatiske overflade af thrombin (se PyMOL info herunder). Foreslå et bindingsite for heparin på overfladen af thrombin.

::: {.callout-solution}
Et sammenhængende blåt område vil sandsynligvis binde heparin da de negative ladninger på heparin vil vekselvirke favorabelt med den positivt ladede overflade.
:::

::: {.callout-note title="PyMOL-info"}

Vi skal nu visualisere den elektrostatiske overflade. 
I PyMOL-interface bruges Action menuen: `A>generate>vacuum electrostatics>protein contact potential (local)`. 
Se figuren forneden.

![](figures/te_12/opgave-4-figur-2-new.png){width="90%" fig-align="center" .lightbox}
...
![](figures/te_12/opgave-4-figur-3-new.png){width="45%" .lightbox}
![](figures/te_12/opgave-4-figur-4.png){width="45%" .lightbox}
...
![](figures/te_12/opgave-4-figur-5-new.png){width="45%" fig-align="center" .lightbox}
...
![](figures/te_12/opgave-4-figur-6.png){width="90%" fig-align="center" .lightbox}
...
6. Lav en scene, kaldet F3, som viser heparins binding til thrombin (PDB-ID: `3UTU`). Stemmer heparins placering overens med det forventede bindingsite?

::: {.callout-tip title="Hint"}
Brug `align` og `util.protein_vacuum_esp`. 
:::
...
```default
{{< include pymol_scripts/script_te13_e4.pml >}}
```
:::

{{< pagebreak >}}

## Opgave 5. Antithrombin (PyMOL) 
...
::: {.callout-solution}
Kræves for at stoppe størkningsprocessen af blod igen. Se Berg 10, s. 228-232.
:::
...
![Effekten af antithrombin III (x-aksen, µM) på factor XIa's proteolytiske aktivitet (ved 8 nM) er angivet langs y-aksen i arbitrære enheder under tilstedeværelse samt uden heparin (0.28 µM).](figures/te_12/opgave-5-figur-1.png){width="80%" fig-align="center" .lightbox}

2. Hvordan  påvirker heparin serpinens aktivitet?

::: {.callout-solution}
Heparin har en kraftig positiv effekt på den grad, hvormed antithrombin III er i stand til at inaktivere faktor XIa.
:::
...
3. Fortsæt scriptet fra opgave 4. Lav en scene, kaldet **F4**, som nedhenter komplekset og viser antithrombin og thrombin i forskellige farver. Lav så en scene, kaldet **F5**, som viser heparins binding til antithrombin/thrombin-kompleksets overflade. Forklarer denne struktur effekten af heparin?

::: {.callout-solution}
Ja, det gør den. Vi kan se at heparin analogen krydsbinder thrombin og antithrombin. Virkeligheden er dog lidt mere kompleks idet man også ved at heparinbinding til antithrombin inducerer konformationsændringer i antithrombin.
:::

Strukturen af factor XIa findes i PDB-ID: `3SOR`. Lav en overlejring af `3SOR` på thrombin i `1TB6`.

4. Lav en scene, kaldet **F6**, der viser XIa's interaktion med heparin. Vil man forvente at heparin binder på samme måde til Factor XIa som til thrombin?

::: {.callout-solution}
Ikke umiddelbart da ladningsfordelingen på overfladen af faktor XIa er anderledes end på overfladen af thrombin. Vi kan dog se at der er andre mulige bindingssteder for heparin på overfladen af faktor XIa og man kan derfor forvente at heparin også kan krydsbinde de to komponenter.

PyMOL script

```default
{{< include pymol_scripts/script_te13_e5.pml >}}
```
:::
...
<pre>
         10         20         30         40         50         60
 MARVRGPRLP GCLALAALFS LVHSQHVFLA HQQASSLLQR ARR<strong>ANKGFL</strong>E EVRKGNLERE
 ...
 GDRLGEDPDP DAAIEG<strong>R</strong>TSE DHFQPFFNEK TFGAGEADCG LRPLFEKKQV QDQTEKELFE
        370        380        390        400        410        420
 SYIEG<strong>R</strong>IVEG QDAEVGLSPW QVMLFRKSPQ ELLCGASLIS DRWVLTAA<strong>H</strong>C LLYPPWDKNF
        430        440        450        460        470        480
 TVDDLLVRIG KHSRTRYERK VEKISMLDKI YIHPRYNWKE NLDR<strong>D</strong>IALLK LKRPIELSDY
        490        500        510        520        530        540
 IHPVCLPDKQ TAAKLLHAGF KGRVTGWGNR RETWTTSVAE VQPSVLQVVN LPLVERPVCK
        550        560        570        580        590        600
 ASTRIRITDN MFCAGYKPGE GKRGDACEGD <strong>S</strong>GGPFVMKSP YNNRWYQMGI VSWGEGCDRD
        610        620
