...
::: {.callout-note}
Dette TØ sæt skal lære jer at bruge kommandolinjen og scripte i PyMOL. Det forudsættes at Microsoft Visual Studio Code er installeret og at I har fulgt PyMOL guide 2 og set Video 3 og 4. TØ opgave 1-3 gennemgås af instruktor på tavlen. TØ opgave 4-6 laves i grupper af de studerende.
:::
...
1. Hent strukturen af K^+^-kanalen fra *Streptomyces* med kommandoen `fetch 1BL8`. Kommandoer tastes nederst i vinduet 
    hvor der står `PyMOL>`. I denne og følgende opgaver på kurset vil følgende angive en kommando, der skal skrives i PyMOL (eller stå i et script):
...
3.  Prøv at vælge selektionen så de små selection handles vises. Sluk nu for hovedobjektet `1BL8` og bemærk at hele
    strukturen forsvinder, mens selektionen stadig vises. Der er ingen atomer i selektionen, så hvis man slukker for hovedobjektet, ses strukturen ikke.
...
5.   Ved at oprette et nyt objekt har vi i praksis lavet en kopi af en del af strukturen (atompositioner etc.), 
    så vi nu har to kæde A. Prøv at skjule det oprindelige objekt 1BL8 (klik på objektnavnet) og se at vi stadig kan se den ene kæde i chain_A. Vi har nu to strukturer af kæde A.
...
1. Hent strukturen af *Streptomyces* K^+^-kanalen (`fetch 1BL8`). Har man allerede startet PyMOL og vil slette alt, kan man skrive kommandoen reinit (reinitialize), eller vælge `File->Reinitialize->Everything` fra menuen.
...
2. Vis 1BL8 som sticks og zoom lidt ind med musen (huske at skjule cartoon-repræsentationen først). Farv den med atomfarver med carbon i gult.
...
3.  Midt i kanalen findes en stor hydrofob aminosyre, Phe 103, nær en kavitet. Phenylringen er rimelig tydelig. 
    Klik med musen for at identificere aminosyren og vælge den i sele. Hvis du rammer forkert, klik da udenfor strukturen for at slette selektionen igen. Når man klikker på et atom, skriver PyMOL:
...
4. Cα hedder altså CA, Oγ hedder OG og så fremdeles. Nogle gange (f.eks. i Asp) kan der være to ens atomer på samme position, hvorved de navngives 1 og 2, f.eks. hedder de to carboxylatatomer i Asp OD1 og OD2.

5. Bemærk at selektionen vælger hele residuet (11 atomer) når man klikker på ét atom. Dette styres i menuen Mouse > Selection Mode. Skift selection mode til "Chains" og brug dette til at vælge hele kæde A. Nu kan vi omdøbe den generelle selektion til noget andet, så vælge "rename selection" i Action-menuen for sele og navngiv den `select_A`. Skift tilbage til selection mode `Residues` og vælg Phe103 i kæde A igen. Bemærk at der nu kommer en ny sele-selektion. På denne måde kan man opbygge selektioner uden brug af kommandoer.
...
6. Vi kan også bruge nomenklaturen ovenfor til at danne en selektion med CG-atomet fra Phe 103:\
...
7. Hvis man kun benytter en del af felterne er reglen at hvis udsagnet starter med / så skal man have alt med fra venstre 
    men må godt stoppe halvvejs. Starter udsagnet ikke med / fortolkes det som at starte fra højre. Eksempler:\
...
8. En anden nyttig regel er at man kan udelade noget af informationen for at vælge alt, f.eks. vil \
...
9. Man kan også angive en range af residues vha. en bindestreg, f.eks. vil\
...
10. Endelig kan man vælge residues i proteinsekvensen. Vælg `Display->Sequence` fra menuen og træk fra residue Arg27-Leu36 i sekvensen for at vælge det. Find selv ud af hvordan man vælger i de andre kæder (Hint: Sæt først kæderne til forskellige farver, da de vises i sekvensen).

::: {.callout-solution}
Kæderne vises efter hinanden i sekvensen. Sætter man farver på kæderne følger sekvensen efter med samme farve. 
:::

11. Brug nu select-statements og nomenklaturen ovenfor til at lave følgende selektioner. Brug forskellige, fornuftige navne til dem alle:
...
12. Brug dine selektioner til at vise hele strukturen i cartoon med en lys grøn farve, farv kæde D mørkegrøn, farv alle Phe-residues i strukturen røde, vis Trp87 i kæde D med sticks og atomet Nε1 med en blå sphere. Vis alle Cα-atomer i kæde A med røde spheres. Farv området 110-120 i kæde B gult.
...
13. Hvad skete der med cartoon-farven i kæde A, da du ændrede farven på Cα-atomerne? Diskutér.

::: {.callout-solution}

Cartoon-farven styres af CA-atomerne, så hvis man farver dem ændrer cartoonen også farve. Løsningen, hvis man vil vise CA og sidekæde i én farve og cartoon i en anden, er at oprettet et nyt objekt (ikke en selektion) med de udvalgte residues. Dette trick benyttes meget ofte til figurer.
...
2. Mange begyndere bliver ofte usikre på om de skal bruge AND eller OR. Hvad ville resultatet være ved at skrive,\
...
::: {.callout-solution}
Vi ville vælge både hele kæde D og residue 87. 
:::

3. På samme måde kan man vælge atomer efter navn (se forrige opgave) med "name" og residues efter navn (f.eks. Trp) vha. "resn". Skriv nu kommandoer til at lave selektionerne fra før og giv dem fornuftige navne,
...
4. Foreningsmængde-operatoren OR benyttes når man skal vælge mere end én ting, f.eks. vælges både kæde A og B således,
...
6. Vis strukturen som ribbon for at lette identifikationen af Cα-atomer. Identificér start og slut residues for den lange, yderste helix i strukturen, der er én i hver kæde, så det er lige meget hvilken du bruger.
Lav en selektion, der indeholder hele den lange helix i alle kæder og farv dem røde. 

::: {.callout-solution}
Cirka residues 23-51:
...
7. Hvis man vil vælge det hele, men undlade en lille del, kan negationsoperatoren NOT være nyttig. F.eks. vil,\
...
color blue, longhelix
```
::: 

8. Lav en selection, der vælger alle residues i kæde A, undtagen Cys-residues.
...
Som det står beskrevet i "PyMOL guide 2", så anbefaler vi, at du laver en "STRBIO" arbejdsmappe til dine PyMOL-scripts i din hjemmemappe. Hvis du vil have, at PyMOL altid skal starte i STRBIO arbejdsmappen, skal du oprette en fil (ved hjælp af VS Code) i din hjemmemappe kaldet .pymolrc, som indeholder kommandoen "cd STRBIO". Under kurset kan du opbygge et sæt scripts i din STRBIO-mappe, som du nemt kan finde på kommandolinjen i PyMOL med "cd " og tab. For at åbne pse-scripts bruges "load [script name].pse". For at åbne en pml-fil skal du skrive "load [script name].pml" eller "@[script name].pml". Hvis du giver dine scripts systematiske navne som "TØX_OpgY_my_script.pml", vil de være nemme at finde. Som med alle kommandolinjer kan du bruge "tab" til at lave "auto-complete" på filnavnene og dermed hurtigt hente alle dine scripts, f.eks. for at vise dem til TØ-præsentationerne.
:::
...
2. Start scriptet med at reinitialisere. Dette er rart, da man næsten altid kører scriptet mange gange under udviklingen og gerne vil have en "ren tavle" uden at genstarte PyMOL:
...
4. Vi ved at PDB-filen kommer ind som cartoon som standard, så lad os fjerne det hele, så vi kan starte på en frisk:

    ```default
    hide all
    ```

5. Skriv kommandoer til oprettelse af selektioner for hver kæde kaldet `chainA`, `chainB`, `chainC` og `chainD`.

6. Skriv kommandoer til at farve kæderne i forskellige farver vha. selektionerne, hertil benyttes kommandoen `color`, f.eks. vil
...
8. Orientér nu strukturen som du gerne vil have den skal se ud (zoom, centrering, rotation). Brug venstre museknap til at rotere, højre museknap til at zoome og tryk på hjulet for at centrere. Skriv nu kommandoen (på kommandolinjen, ikke i scriptet):

    ```default
    get_view
    ```

    PyMOL giver os nu en 3x6 matrix, der fuldstændigt definerer synsvinklen, som det output der er vist nedenfor. Kopiér følgende,
...
9. Afhængig af hvordan, du har skrevet scriptet vil du muligvis opleve, at der vises en resterende selektion, når scriptet er kørt. For at undgå dette og fjerne den sidst valgte gruppe af atomer, indsæt kommandoen:
    ...
    Dette gemmer den nuværende visning i en scene kaldet `overview`. Bemærk at man ikke kan bruge F-tasterne for at frembringe denne scene, da den er navngivet noget andet end F"-". Man kan derfor i stedet enten trykke på "Scenes" menuen i øverste højre hjørne og finde scenen med navnet `overview` eller man kan skrive kommandoen: `scene overview, recall`.

10. Du har nu et komplet PyMOL-script, der først initialiserer, så henter PDB-filen, definerer selektioner og bruger dem og endelig sætter repræsentation og view. Test scriptet ved at kopiere hele teksten og indsætte den på PyMOL's kommandolinje (nederst i vinduet). Tryk Enter/Return for at køre scriptet.

11. Prøv at orientere molekylet på en anden måde og tjek at du kommer tilbage til start ved at klikke `overview` i "Scenes" menuen.

12. Nu skal vi oprette et nyt view, der viser detaljer i K^+^-kanalens såkaldte selektivitetsfilter, der er med til at sikre, at kun K^+^-ioner kan passere. Denne del af scriptet skrives efter ovenstående og vil blive gemt i en ny scene. Opret først en ny selection med ionerne:\
...
13. Vis ionerne som non-bonded spheres (nb_spheres), der er mindre end alm. `spheres`, der viser VDW-radius:\
...
14. Opret nu en selektion med kanalens selektivitetsfilter. Dette ligger i residues 75-78 i alle kæder, så vi kan nøjes med at skrive:\
...
16. Scriptet skal vise `overview` først, så afslut af med at hente den ind igen:

    ```default
    scene overview, recall
    ```

17. Kør det samlede script i PyMOL og check at der er to views, som man hurtigt kan skifte i mellem.

::: {.callout-solution}
Eksempel på færdigt script:

```default
{{< include pymol_scripts/script_te3_e4.pml >}}
```
:::
...
I denne opgave skal vi kigge på et eksempel, der er nævnt i kapitel 5 i Berg 10. udgave, strukturen af cytochrome P450 bundet til substratet camphor (Fig. 5.5):

![](figures/te_03/opgave-5-figur-1x.png){width="80%"  fig-align="center" .lightbox}
...
1. Start et ny PyMOL-script i VS Code med at reinitialisere og hent dernæst strukturen ovenfor via PDB-koden 2CPP, kald objektet cytochrome og skjul alle repræsentationer:
...
3. Nu skal vi lave en selection med substratet, men vi ved ikke hvad det er kaldet i PDB-filen. Et godt tip til at finde ud af dette er at slå strukturen op på RCSB: 

    [https://www.rcsb.org/structure/2cpp](https://www.rcsb.org/structure/2cpp). 

    Under "Small molecules" kan man se at der både findes en hæme prostetisk gruppe (kaldet HEM) og liganden camphor (CAM). Disse tre-bogstavsforkortelse henviser til det, der står ud for "residue name" i PDB-filen, dvs. at vi skal bruge kommandoen "resn" til at vælge dem:
...
4. Vi vil også gerne have en selektion med alle de sidekæder, der interagerer med substratet, hvilket kan ses af figuren fra lærebogen:
...
9. Indsæt nu en kommando så ingen atomer er valgt og find et passende view, der sætter dig i stand til at analysere interaktionerne i det aktive site. Husk at du kan bruge midterste museknap til at centrere, f.eks. på camphor, og hjulet til at skjule ikke-relevante dele af proteinet.

10. Find også en god oversigtsorientering og gem de to som scener under `overview` og `active site`.
...
11. Brug nu `active site`-scenen til at analysere interaktionerne mellem cytochrome P450 og camphor/heme. Hvilke typer interaktioner findes der? Hint: Brug `Wizard -> Measurement` til at måle afstande. 

::: {.callout-solution}
Samlet script fra underopgaver **1-10**.

```default
{{< include pymol_scripts/script_te3_e5.pml >}}
```
...
1. Slå strukturen op på [**www.rcsb.org**](http://www.rcsb.org) og hent den ind i PyMOL.
...
::: {.callout-solution}
Interaktionerne er ikke meget specifikke, så der er flere muligheder. Et svar kunne være resterne 104, 70, 11, 145, 106, 26, 35, 166, 114 og 137.
:::

2. Opret et PyMOL-script, der gør følgende:

    - Initialiserer og henter strukturen ind i programmet som `148L`.

    - Skjuler alt.

    - Opretter selektioner for lysozym og de to substrater.

    - Viser lysozym som cartoon i `violetpurple`.

    - Viser substraterne som sticks i to forskellige farver for kulstof og atomfarver for de andre atomer.
    ...
    - Gemmer både en oversigts- og to detaljevisninger i tre scenes, `overview` samt `cell wall` og `sugar`, der fokuserer hhv. på de to substratdele.

::: {.callout-solution}
Eksempel på script:
```default
{{< include pymol_scripts/script_te3_e6.pml >}}
```
:::

3. Brug dit script til at analysere interaktionerne mellem lysozym og de to substrater. Hvilke typer interaktioner findes der? Hvilken aminosyrerest er kovalent forbundet til substrat?

::: {.callout-solution}
Adskillige både hydrofobe og H-bindingsinteraktioner. Glu26 binder kovalent til sukkerdelen.
:::