...
**top menu** provides a set of handy buttons for quick access to various
functions. The main **display** **panel** provides the visualization of
the loaded molecules while the **object panel** on the right-hand side
...
achieved using the **command line** located at the bottom of the window.
The **log window** contains log of which commands have been executed and
details of selections. Note also the **bottom menu** of the object menu
that contains controls for mouse, wizards, sequences and movies and a
button to hide/show the log window.

![](pymol_guide_1_media/media/image1.png)
...
functionality attached. You can use the \"bottom menu\" to set the mouse
mode and see a guide to the different functions that are available (see
figure below).

![](pymol_guide_1_media/media/image2.png)

Most tasks in PyMOL can be achieved using the \"object panel\" on the
right-hand side of the \"display panel\". Here, each line (or entry) in
the menu correspond to the objects you have loaded into PyMOL (e.g.
...
![](pymol_guide_1_media/media/image3.png)
...
You can also find the wizard tools in the \"bottom menu\" next to the
mouse button. This will display the 8 different molecular
...
The PyMOL \"display panel\" will now show the 3D structure of *E. coli*
LacZ in the default cartoon representation with the ligand shown as
sticks colored by atom type. Note that the \"object panel\" now contains
an entry named **5a1a** representing our newly created object.
...
Click the **Zoom** button in the \"top menu\" to zoom on all or selected
...
panel\". Click the **A** next to the **5a1a** object. From the menu that
...
menu\" and choose \"Next Ligand\", which will zoom to a view of an
individual ligand.
...
![](pymol_guide_1_media/media/image4.png)**Figure 2**: Example of view of PETG
bound to *E. coli* LacZ.
...
object menu (**A** → **preset** → **ligand sites**). See in particular
the following:
...
## ![](pymol_guide_1_media/media/image5.png)Displaying molecular representations 
...
the **S** (show) button from the object menu next to the **5a1a** entry.
...
clicking the particular atom in the \"display panel\".
...
in the \"display panel\" with small pink squares.
...
should now appear as sticks in \"display panel\".

![](pymol_guide_1_media/media/image6.png)Even though we initially only clicked at
...
button\" in the left side of the \"top menu\" (named \"Residues\" by
default). If you choose **Atoms**, you will be able to select a single
atom when clicking on it in the \"display panel\". Now zoom out to see
...
when clicking on it in the \"display panel\". Choose **Residues** again
before continuing.
...
the \"bottom menu\". When clicking **SEQ** once you will see the
...
To disable the selection, click on an empty part in the \"display
panel\", or click the **(sele)** entry in the object menu.
...
![](pymol_guide_1_media/media/image8.png)While the cartoon representation displays
...
![](pymol_guide_1_media/media/image9.png)To show potential hydrogen bonds use
...
contacts. Do this by clicking the wizard button in the \"bottom menu\"
...
## ![](pymol_guide_1_media/media/image10.png)Ray tracing for better resolution 
...
Use the \"Draw/Ray\" button in the \"top menu\" to make a ray tracing of
...
![](pymol_guide_1_media/media/image11.png)Personal preference obviously plays a role,
but here are some additional settings that can be used to make a nice
